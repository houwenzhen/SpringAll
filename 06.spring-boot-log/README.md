...
### 介绍
### logger_filer
>每一个Java程序员都知道日志对于任何一个Java应用程序尤其是服务端程序是至关重要的，而很多程序员也已经熟悉各种不同的日志库，如java.util.logging、Apache log4j、logback。但如果你还 不知道SLF4J（Simple logging facade for Java）的话，那么是时候在你的项目中学习使用SLF4J了。
...
```
#### 1.2创建日记配置文件
>src/main/resources/目录下创建logback.xml 日记配置文件 参数说明: 1.logDir 自定义日记输出位置，默认项目相对路径 ：xx项目/log 2.maxHistory 日志最大保存时限 xml 完整配置文件：
...
#### 2.3 application.properties中加入配置
```
...
#### 2.5 改造logger_filer
>改造logger_filer ，用于接收 client发送的日记
